...
const createSalesReport = async (startDate,endDate)=>{
  try {
    ...
    const totalSales = Math.floor(productProfitsArray.reduce(
      (total,product) => total + product.profit,0
    ));
  ...
  try {
    ...
    for (const order of orders ) {
      for (const productInfo of order.products) {
        ...
        const userDetails =await User.findById(order.userId).select("email firstName")
...
//=================== portfolio chart data filtering =============================// 

const portfoliofiltering = async (req,res )=> {
  try {
    let datePeriod = req.body.date;
